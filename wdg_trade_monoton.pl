...
while (1)
{
	my $filename = 'poloniex_trade_monoton_wdg.txt';
	...
	if ( $last_line ne "" )
	{
		if ( $last_line ne "start" )
		{
			...
			if ( ($crtTime - $lastTime) > 70 )
			{
				...
				# restart the poloniex_trade_monoton script
				 print "kill poloniex_trade_monoton becuase distance to the last time is $delta seconds \n";
				 my $pid = `ps -ef | grep poloniex_trade_monoton.pl | grep -v grep | awk '{print \$2}'`;
