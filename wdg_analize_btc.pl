...
while (1)
{
	my $filename = 'poloniex_analize_btc_wdg.txt';
	...
	if ( ($crtTime - $lastTime) > 60 )
	{
		...
		# restart the poloniex_get_data script
		 print "kill poloniex_analize_btc becuase distance to the last time is $delta seconds \n";
		 my $pid = `ps -ef | grep poloniex_analize_btc.pl | grep -v grep | awk '{print \$2}'`;
